...
public:
    vector<int> maxActiveSectionsAfterTrade(string s, vector<vector<int>> &queries)
    {
        ...
        for (auto &q : queries)
        {
            // har aik query k leye same  step karo
            vector<int> sections;
            int l = q[0];
            int r = q[1];
            int zeroCount = 0;
            ...
            {
                best = max(best, sections[s] + sections[s - 1]);
            }
            ans.push_back(best + totalOnes);
        }
        return ans;
    }
};
...
public:
                                            ...
                                            vector<vector<int>> &queries)
    {
        ...
        if (m < 2)
        { // continuous 0 blocks less than 2 segments, return the
          // answer directly
